...
class NewsPaperClass {
  ...
  Future<LatestNewsModel> latestNews() async {
    var response = await http.get(
        Uri.parse(NewsConfig.newsurl + NewsConfig.newslatest));
  ...
  Future<CategoryNewModel> getcategory() async {
    var response = await http.get(
        Uri.parse(NewsConfig.newsurl + NewsConfig.categoryNews));
    if (response.statusCode == 200) {
  ...
  Future<SearchModel> getSearch(String item) async {
    var response = await http.post(
        Uri.parse(NewsConfig.newsurl + NewsConfig.search),
        body: <String, String>{
          'search': item
        });
    if (response.statusCode == 200) {
  ...
  Future<bool> signOut(String token) async {
    var response = await http.get(
        Uri.parse(Config.serverUrl + Config.logoutProfile),
  ...
  Future<CategoryDetailsModel> getCategoryNewsdetails(String id) async {
    var response = await http.get(
        Uri.parse('${NewsConfig.newsurl + NewsConfig.categoryDetails}/$id'));
    var data = jsonDecode(response.body);
    if (response.statusCode == 200) {
  ...
  //
  // Future<GetCommetnModels> getComment(String id) async {
  //   var response = await http.get(
  //       Uri.parse('${NewsConfig.newsurl + NewsConfig.getComment}/$id'));
  //   var data = jsonDecode(response.body);
  //   if (response.statusCode == 200) {
  //     return GetCommetnModels.fromJson(data);
  //   }
  //   else {
  //     throw Exception('Error');
  //   }
  // }
  //
  // Future<PostCommentModel> postComment() async {
  //   var response = await http.get(
  //       Uri.parse(NewsConfig.newsurl + NewsConfig.postComment));
  //   if (response.statusCode == 200) {
  //     return PostCommentModel.fromJson(jsonDecode(response.body));
  //   }
  //   else {
  //     throw Exception('Error');
  //   }
